...
def divide_data(data, feature_column, feature_val):
    ...
    data1=[]
    data2=[]
    if type(feature_val) == int or type(feature_val) == float or type(feature_val)==np.float64 or type(feature_val)==np.int64:
        for row in data:
...
def gini_impurity(data1, data2):
    ...
    N1*sum(p_1k * (1-p_1k)) + N2*sum(p_2k * (1-p_2k))
    ...
    for k in set(y1):
        p1[k]=y1.count(k)/float(N1)
        
    for k in set(y2):
        p2[k]=y2.count(k)/float(N2)
    
    
    return N1*sum([p1[k]*(1-p1[k]) for k in set(y1)])+N2*sum([p2[k]*(1-p2[k]) for k in set(y2)])
...
def build_tree(data, current_depth=0, max_depth=1e10):
    ...
    if(current_depth == max_depth):
        M=max(dict_of_values(data).values())
        for i in dict_of_values(data).keys():
            if dict_of_values(data)[i]==M:
                best_result=i
        return DecisionNode(current_results=dict_of_values(data), result=best_result)
    ...
    if(len(dict_of_values(data)) == 1):
        ...
        for i in dict_of_values(data).keys():
            if dict_of_values(data)[i]==M:
                best_result=i
        return DecisionNode(current_results=dict_of_values(data), result=best_result, is_leaf=True)
    ...
    for feature in range(0,num_features,8):
        for value in set(np.array(data)[:,feature]):
            if gini_impurity(divide_data(data, feature, value)[0],
                            divide_data(data, feature, value)[1])<best_gini:
                best_gini=gini_impurity(divide_data(data, feature, value)[0],
                            divide_data(data, feature, value)[1])
    ...
    t1=divide_data(data, best_column, best_value)[0]

    t2=divide_data(data, best_column, best_value)[1]
    
    t1=build_tree(data=t1,current_depth=current_depth+1,max_depth=max_depth-1)
    t2=build_tree(data=t2,current_depth=current_depth+1,max_depth=max_depth-1)
    ...
    return DecisionNode(column=best_column, value=best_value, true_branch=t1, 
                        false_branch=t2, current_results=dict_of_values(data), result=best_result)
